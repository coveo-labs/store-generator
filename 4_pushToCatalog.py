...
def fixImages(images, config, baseurl):
  newimages=[]
  for image in images:
    #..\\images\\hat\\601168.jpg
    newimages.append(image.replace('..\\images\\',baseurl).replace('\\','/'))
  return newimages
...
def process(json, allcolors, config, baseurl, baseurlimages,child):
  #create final json
  ...
  images=fixImages(images, config, baseurlimages)
...
def storeJson(jsond,config):
  try:
       with open(config['global']['catalogJsonFile'], "w", encoding='utf-8') as handler:
    ...
    while imgnr<len(urls):
      ...
      if ('_' not in image):
        ...
        if 'colorxy' in json:
          total=total+1
          kids=False
          json=process(json,allcolors,config['categories'][catnr],config["global"]["baseUrl"],config["global"]["baseUrlImages"],None)
          if 'childs' in json:
            for child in json['childs']:
                json=process(json, allcolors, config['categories'][catnr],config["global"]["baseUrl"],config["global"]["baseUrlImages"],child)
