...
def pick_color(event,x,y,flags,param):
  ...
  if event == cv2.EVENT_LBUTTONDOWN:
        ...
        #print ("current")
        #print (currentFile)
        rec = createCopiesImages(currentFile, currentJson, 0, image_normal,'','')
...
def pick_option1(event,x,y,flags,param):
  ...
  if event == cv2.EVENT_LBUTTONDOWN:
      ...
      rec={}
      #photo,rec, childnr, childimage, color,colorhex):
      rec = createCopiesImages(currentFile, currentJson, 1, image_option1, color,hex)
      #updateJson(currentFile,rec,False)
...
def pick_option2(event,x,y,flags,param):
  ...
  if event == cv2.EVENT_LBUTTONDOWN:
      ...
      rec={}
      #photo,rec, childnr, childimage, color,colorhex):
      rec = createCopiesImages(currentFile, currentJson, 2, image_option2, color,hex)
      #updateJson(currentFile,rec,False)
...
def pick_option3(event,x,y,flags,param):
  ...
  if event == cv2.EVENT_LBUTTONDOWN:
      ...
      rec={}
      #photo,rec, childnr, childimage, color,colorhex):
      rec = createCopiesImages(currentFile, currentJson, 3, image_option3, color,hex)
      #updateJson(currentFile,rec,False)
...
def createCopiesImages(photo,rec, childnr, childimage, color,colorhex):
    ...
    if height>h:
      height=h
    newimage2 = originalimage[y:height, 0:w]
...
def processImages(filename):
    ...
    while imgnr<len(urls):
      ...
      if ('_' not in image):
        if (prevnr!=imgnr):
          ...
          prevnr = imgnr
        print( "")
        print(" Click on the image where the "+cat.upper()+" is !!!")
        print( "")
        currentFile = image
  ...
  print("NEXT STEP: 4. Push Catalog.")
