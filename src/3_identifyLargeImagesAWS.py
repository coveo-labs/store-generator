...
def detect_labels(photo, settings, config):
...
def process(image, settings, skipAlreadyDone, config):
    ...
    if 'AWS' in data and skipAlreadyDone:
      ...
      #take the _0 photo
      photo = image.replace('.jpg','_0.jpg')

      if (os.path.isfile(photo)):
        rec=detect_labels(photo, settings, config)
...
def processImages(filename):
  ...
  while catnr<len(config['categories']):
    ...
    for image in urls:
      if ('_' not in image):
        ...
        total=total+1
        data = process(image,settings, config['global']['AWSSkipExisting'], config)
  ...
  print("NEXT STEP: Push to catalog: 4_pushToCatalog")
